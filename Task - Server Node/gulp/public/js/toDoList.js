...
class ToDo {
    ...
    async _loadFromServer(id) {
        const response = await fetch(id ? `${ORIGIN}/todos/${id}` : `${ORIGIN}/todos`, {
            method: "GET"
        });
    ...
    async _updateToServer(todo) {
        await fetch(`${ORIGIN}/todos/${todo.id}`, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
